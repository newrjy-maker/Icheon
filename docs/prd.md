...
1. 보호자가 여행자 연령대, 사용 가능한 시간, 관심사를 선택하고 시작 지역 맵을 고른다.
...
- 하나의 지역 맵은 서로 차량 이동 10분 이내로 연결되는 서로 다른 퀘스트 스팟 **정확히 3곳**으로 구성하고 권장 방문 순서 1→2→3을 제공한다.
...
- 현재 지역 맵에 포함된 세 퀘스트는 어느 장소부터든 현장 QR로 인증할 수 있다. 같은 퀘스트를 반복해도 조각은 한 번만 지급하며 진행도는 `0/3`부터 `3/3`까지 표시한다.
...
- AC1: 여행자 연령대, 남은 시간, 관심사를 입력하면 선택 가능한 시작 지역 맵이 표시된다.
...
- AC3: 현재 지역 맵에 포함되지 않은 QR을 스캔하면 완료 처리하지 않고 현재 지역 맵의 인증 가능 장소를 안내한다.
...
1. **계정·시작 설정** — 서비스 전용 아이디로 가입·로그인하고 연령대, 사용 가능 시간, 관심사를 선택한다.
...
- **사용자 세션:** 계정 ID, 로그인 세션 토큰, 연령대, 관심사, 남은 시간
...
- 연령대·관심사·남은 시간은 현재 로그인 세션에서 추천에만 사용하고 계정에 장기 저장하지 않는다. 로그아웃 또는 세션 만료 시 폐기한다.
...
|---|---|---|---|---|
| 동 | 도자 예술맵 | 이천도자예술마을(예스파크) → 사기막골도예촌 → 월전미술관 | 만 3~10세·도자+창작 | 1→2, 2→3 각 10분 기준, QR 거점·체험 예약 여부 |
...
2. 권장 방문 순서는 1→2→3으로 제공하며, 1→2와 2→3의 기준 차량 이동 시간은 각각 10분 이하여야 한다. QR 인증은 세 장소 중 어느 곳에서나 가능하다.
...
8. 여행자 연령대에 맞지 않는 산악·레저 장소와 예약 필수 장소는 시작 맵 추천에서 제외한다.
